...
全局边界见 `AGENTS.md`，共同执行协议见 `指导/使用说明.md`，具体做法见本文列出的
专项指导。
...
阶段编号只表示推荐主线，不是门禁。用户可以直接进入任意技术上可执行的阶段，也可以
明确跳过任意阶段。
...
“文件存在”不等于“已完成”。只有标题、占位符、无依据草稿或未记录必要确认的文件
不是有效产物。

跳过也不等于完成。用户要求跳过时，Agent 先说明影响，把决定、原因和证据限制写入
目标阶段或当前交接文档，然后继续用户指定的目标。只有目标行为本身技术上无法完成时
才标记阻塞，不能用推荐前序阶段强迫用户绕回。
...
2. 定位已有 `workspace/<中文任务名>/`，或按第 3 节处理新任务。
3. 通读任务目录 Markdown，再按需检查稿件、论文材料、实验记录和独立实验仓库。
...
当前阶段是“用户当前主线所在的位置”，不是编号最大的文件。当前能力是“此刻被调用
的专项操作”，调用完成后返回主线或结束一次性任务。

## 3. 任务初始化

`workspace/` 使用扁平结构，每个任务直接位于 `workspace/<中文任务名>/`。不创建
固定类型中间层，也不创建 `任务状态.md`。

### 3.1 新学习任务

先确认中文任务名、领域范围、学习目标和首次产物。用户确认首次落盘后创建目录，只
创建 L1 当前需要的文档。

### 3.2 新研究 Idea

R1 的初步审查先在对话中完成，不因收到一个原始 Idea 就创建永久研究目录。只有本质
假设、撞车和硬度审查通过，并且用户确认立项后，才创建中文研究任务目录和 R1 正式
产物。Idea 被否决且用户停止时，任务结果是终止，不是阻塞。

若用户指定了已有目录，则在该目录中审查和回写，不重复初始化。

### 3.3 自动发现的候选

`自动发现研究想法候选.md` 保存在当前学习或发现任务中。用户选中候选后进入 R1
预审；R1 通过并确认立项前，不创建独立研究目录。

### 3.4 独立能力任务

审稿、翻译、文本修订、引用核验、Tex 验收、匿名检查等能力可以独立运行。没有明确
输出位置时，先确认中文任务名、输入和首次落盘位置；不得为套用生命周期而补建无关
阶段文件。
...
- 中文任务目录保存研究级计划、实验记录、失败记录和结果分析，并引用仓库路径、
  commit、命令、配置和实际产物位置。
...
| 获取或处理论文材料 | 对应文献或论文处理能力 | 目标论文、合法来源、处理范围和传输许可 |
...
R3-R6 面向计算机科学经验研究。非实验论文或用户明确要求时可以跳过，但 R7 不得把
缺失实验写成已支持的 Claim。
...
能力可以在同一阶段反复调用。综合审稿后可以调用绘图或修订，再次审稿；投稿规则核验
通常在 R7 前期和 R8 终稿各执行一次，并持续更新同一份 `投稿规范检查.md`。
...
- **输入**：可执行的实验计划、目标 Claim、用户准备的实验仓库，以及当前可用的研究
  与设计文档；跳过项的影响必须明确。
- **产物**：独立实验仓库中的工程产物；中文任务目录中的 `实验记录/` 和
...
| 论文解析、翻译、重排与工具配置 | `指导/学习/论文处理工具使用.md`、`指导/学习/论文处理工具配置.md` | 处理材料与 `论文处理记录.md` |
...
仓库不主动进行伦理评价。只有 venue 官方明确要求某项声明、表单或披露时，才把它
作为“官方强制声明”核验是否存在和符合格式，不评价其伦理内容。
...
只有在回答问题、构造 Story、比较论文或核验事实时发现证据不足，才定向获取证据并
更新 K 条目。不新增“精读阶段”，也不把人的阅读过程纳入状态机。

研究项目采用 K 条目时，必须把原始来源和准确位置同步到项目 `文献证据库.md`，并记录
来源 K 编号。领域地图的二手总结不能单独作为论文引用依据。
