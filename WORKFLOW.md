...
`workspace/<中文研究项目名>/`。不创建 `学习/`、`研究/` 等固定类型中间层，也不
创建 `任务状态.md`。
...
“归档”至少包括保留原始文件、记录来源与版本、计算文件哈希、记录归档日期，并建立
可追溯的 `论文信息.md` 和 `论文处理记录.md`。PDF 翻译、阅读笔记和评议都继续写入
同一个论文目录。

### 3.3 新研究 Idea

R1 的初步审查先在对话中完成，不因收到一个原始 Idea 就创建永久研究目录。只有本质
假设、撞车和硬度审查通过，并且用户确认立项后，才创建中文研究项目目录和 R1 正式
...
输出位置时，先确认输入、归属和首次落盘位置；不得为套用生命周期而补建无关阶段
文件。纯文本临时处理可以使用用户指定位置；涉及论文归档时必须遵守第 3.2 节。
