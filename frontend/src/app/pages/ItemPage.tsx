...
export function ItemPage() {
  ...
  const [readerMode, setReaderMode] = useState<"pdf" | "semantic">(() =>
    window.localStorage.getItem("reader-mode") === "semantic" ? "semantic" : "pdf",
  );
  ...
  const resource = useApiResource(
    () => itemId ? Promise.all([api.item(itemId), api.attachments(itemId)]) : Promise.reject(new Error("文献地址无效")),
    [itemId],
  );
  ...
  const [item, attachments] = resource.data;
